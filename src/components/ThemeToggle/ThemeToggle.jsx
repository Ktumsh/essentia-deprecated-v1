...
export function ThemeToggle({
  buttonClass = "dark:bg-wash-full-dark",
  spanClass = "dark:bg-wash-dark",
}) {
  ...
  return (
    <button
      type="button"
      className={`group btn__mode relative inline-flex flex-shrink-0 items-center justify-start w-14 h-8 px-1 text-link-h dark:text-gray-400 bg-gray-200 rounded-full overflow-hidden cursor-pointer ${
        theme === "dark" ? "darkmode" : ""
      } ${buttonClass}`}
    ...
    >
      ...
      <span
        className={`btn__indicator flex justify-center items-center size-6 rounded-full bg-white shadow-[0_2px_10px_rgba(0,0,0,.2)] dark:shadow-[0_2px_10px_rgba(0,_0,_0,_0.3)] transition-[margin,_width] z-10 group-active:w-7 ${
          theme === "dark" ? "mr-6 group-active:mr-5" : "ml-6 group-active:ml-5"
        } ${spanClass}`}
      ></span>
      <MoonIcon
        className={`${
