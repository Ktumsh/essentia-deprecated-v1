    ...
    <aside className="hidden lg:block sticky top-0 right-0 flex-grow p-2 w-full max-w-[350px] max-h-dvh mr-16 text-base-color dark:text-base-color-dark">
      <form className="relative w-full mb-3">
        <div className="flex items-center bg-white dark:bg-base-dark rounded-full">
          ...
          <input
            type="text"
            placeholder="Buscar"
            className="w-full h-11 px-5 bg-transparent text-sm placeholder-base-color-m dark:placeholder-base-color-dark-m outline-none ring-0 border-0 focus:outline-none focus:ring-0"
          />
        </div>
      </form>
      <div className="mb-6 bg-white/50 bg-bento-gradient dark:bg-none dark:bg-transparent border border-gray-100/50 dark:border-base-dark backdrop-blur dark:backdrop-blur-none backdrop-saturate-150 dark:backdrop-saturate-100 shadow-md dark:shadow-none text-base-color dark:text-base-color-dark rounded-xl overflow-hidden">
        ...
        {trends.map((trend, index) => (
          <Button
            className="flex-col items-start h-full py-3 data-[pressed=true]:scale-[1]"
            color="danger"
            key={index}
            radius="none"
            variant="light"
